...
from app.schemas.transfer import SignTransferRequest, SignTransferResponse, TransferRequest, TransferResponse
...
from app.utils.signing_helper import build_transfer_payload_hash
...
@router.post("/sign_transfer", response_model=SignTransferResponse)
def sign_transfer(payload: SignTransferRequest) -> SignTransferResponse:
    try:
        expected_public_key = CryptoService.public_key_from_private(payload.private_key)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="Invalid private key") from exc

    if expected_public_key != payload.from_public_key:
        raise HTTPException(status_code=400, detail="Private key does not match the sender public key")

    document_hash = sha256_hex(payload.document_text.encode("utf-8"))
    payload_hash = build_transfer_payload_hash(
        ...
        timestamp=payload.tx_timestamp.isoformat(),
    )
    signature = CryptoService.sign_payload(payload.private_key, payload_hash)
    return SignTransferResponse(payload_hash=payload_hash, signature=signature)
